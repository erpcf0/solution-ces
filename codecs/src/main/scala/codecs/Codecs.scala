...
trait DecoderInstances:
  ...
  given listDecoder[A](using decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      x => x match
        case Json.Arr(x) => x.foldLeft(Some(List.empty[A]): Option[List[A]]) { (acc, cval) =>
          val d = decoder.decode(cval)
          if (d == None || acc == None) then {
            None
          } else {
            Some(acc.getOrElse(List.empty[A]) ++ d)
          }}
        case _ => None
    }
  ...
  def field[A](name: String)(using decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction { x => x match
      case Json.Obj(x) => x.get(name).flatMap { y => decoder.decode(y) }
      case _           => None
    }
...
trait PersonCodecs:
  ...
  given Decoder[Person] =
    Decoder.field[String]("name").zip(Decoder.field[Int]("age")).transform { x => Person(x._1, x._2) }
...
trait ContactsCodecs:
  ...
  given Encoder[Contacts] =
    ObjectEncoder.field[List[Person]]("people").transform { x => x.people }
  // ... then implement the decoder
  given Decoder[Contacts] =
    Decoder.field[List[Person]]("people").transform { x => Contacts(x) }
