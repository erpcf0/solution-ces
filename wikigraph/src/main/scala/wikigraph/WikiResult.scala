...
object WikiResult:
  ...
  /** Asynchronously and non-blockingly transforms a `Seq[A]` into a
    ...
    * combine all the `WikiResult` values with the operation `zip`. If the input
  ...
  def traverse[A, B](as: Seq[A])(f: A => WikiResult[B])(using ExecutionContext): WikiResult[Seq[B]] =
    as.map(f)
      .foldLeft(WikiResult.successful(Seq.empty))((acc, ce) => {
        acc.zip(ce.map(Seq(_))).map(e => e._1 ++ e._2)
      })
