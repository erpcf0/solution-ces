...
final class Wikigraph(client: Wikipedia):
  ...
  def namedLinks(of: ArticleId): WikiResult[Set[String]] =
    client
      .linksFrom(of)
      .flatMap(id => {
        WikiResult.traverse(id.toSeq)(client.nameOfArticle).map { e => e.toSet }
      })
  ...
  def breadthFirstSearch(start: ArticleId, target: ArticleId, maxDepth: Int): WikiResult[Option[Int]] =
    ...
    def iter(visited: Set[ArticleId], q: Queue[(Int, ArticleId)]): WikiResult[Option[Int]] =
      if (q.isEmpty) WikiResult.successful(None)
        ...
        val ((cd, ca), re) = q.dequeue
        if (cd >= maxDepth) WikiResult.successful(None)
        else {
          client
            .linksFrom(ca)
            .flatMap(id => {
              if (id.contains(target)) then WikiResult.successful(Some(cd))
              else
                iter(
                  visited + ca,
                  re
                    .enqueueAll(
                      id
                        .filter(aid => !visited.contains(aid))
                        .map(aid => (cd + 1, aid))
                    )
                )
            })
            .fallbackTo(iter(visited + ca, re))
        }
      }
  ...
  def distanceMatrix(titles: List[String], maxDepth: Int = 50): WikiResult[Seq[(String, String, Option[Int])]] =
    ...
    WikiResult.traverse(allArticlePairs) { (sc, dis) =>
      client.searchId(sc).zip(client.searchId(dis)).flatMap {
        (sourceId, distId) =>
          breadthFirstSearch(sourceId, distId, maxDepth).flatMap { distance =>
            WikiResult.successful(sc, dis, distance)
          }
      }
    }
end Wikigraph
